...
# 1) Inputs / Parameters
############################################################
# input_file              <- "/Users/JJOHN41/Downloads/kadoorie_biobank/mtag_output/fastasset/fastasset_inputs/fastasset_testrun_fastasset_ldsc_input.txt"
# snps_to_munge            <- "/Users/JJOHN41/Documents/software_resources/resourses/postgwas/1000GP_Phase3/eur_w_ld_chr/w_hm3.snplist"
# ld_ref_pannel            <- "/Users/JJOHN41/Documents/software_resources/resourses/postgwas/1000GP_Phase3/eur_w_ld_chr/"
...
# ldsc_maf_cut_of          <- 0.01
# ldsc_infscore_cut_of     <- 0.9

# meth.pval                <- "DLM" 
# fastasset_scr_pthr       <- 0.999999   # screening threshold for fastASSET
# chunk_size               <- 100000
# ncores                   <- max(1, floor(parallel::detectCores() * 0.5))

##--------------------------------------------------------------------------
# ⭐ 1️⃣ Function — capture R + packages + system info


# ------------------------------------------------------------
# Helper: Detect script directory (CLI-safe, Docker-safe, HPC-safe)
# ------------------------------------------------------------
...
# ⭐ ⭐ ⭐ MASTER FUNCTION (orchestrator)
run_asset_pipeline <- function(params) {

  suppressPackageStartupMessages(library(data.table))

  # ------------------------------------------------
  # 0) Directory structure (prefer params-derived)
  # ------------------------------------------------
  ...
  # ------------------------------------------------
  # 4) Load fastASSET input (ONE copy)
  # ------------------------------------------------
  asset_input_df <- safe_stage(
    "load_fastasset_input",
    quote(data.table::fread(params$fastasset_input)),
    report_env
  )

  # ------------------------------------------------
  # 5) Trait list + LDSC cor + blocks
  # ------------------------------------------------
  beta_cols <- grep("\\.Beta$", names(asset_input_df), value = TRUE)
  if (length(beta_cols) == 0) stop("No .Beta columns found in fastasset_input")

  traits <- sub("\\.Beta$", "", beta_cols)

  if (!all(traits %in% colnames(LDSCoutput$I))) {
    missing <- setdiff(traits, colnames(LDSCoutput$I))
    stop("Traits in fastasset_input missing from LDSCoutput$I: ",
  ...
  # ------------------------------------------------
  # 6) fastASSET stage (STREAMING, no beta_dt/se_dt/n_dt)
  # ------------------------------------------------
  safe_stage(
    "fastasset",
    quote(run_fastasset_stage(
  ...
  # ------------------------------------------------
  # 7) h.traits stage (pass run_name)
  # ------------------------------------------------
  safe_stage(
    "htraits",
    quote(run_htraits_stage(
      asset_input_df = asset_input_df,
      LDSC_cor       = LDSC_cor,
      trait_names    = traits,
  ...
  # ------------------------------------------------
  # 8) Environment + tool capture
  # ------------------------------------------------
  safe_stage(
    "environment_capture",
    quote(capture_environment(params$output_dir)),
    report_env
  )

  safe_stage(
    "external_tools_capture",
    quote(capture_external_tools(params$output_dir)),
    report_env
  )

  message("✅ Pipeline completed successfully")

  # Optional: return report summary
